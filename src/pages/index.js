...
export async function getStaticProps() {
  const result = await fetch("http://localhost:4000/api/metaBlog/meta");
  const posts = await result.json();

  const trendings = await fetch("http://localhost:4000/api/metaBlog/trendings");
  const trending = await trendings.json();

  const posts2 = await fetch("http://localhost:4000/api/metaBlog/blog");
